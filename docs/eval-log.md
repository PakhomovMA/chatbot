...
инструменты**: модель в этой среде не соблюдает протокол вызова инструментов Ollama.
...
Выводы: (1) падение e2e не вызвано этими инструментами — baseline падает раньше и хуже (ноль вызовов
против двух отвеченных вопросов); (2) модель понимает новый инструмент и тянется к нему, но упирается в
tool-protocol, а не в описание инструмента — тот же сбой ловит и старый `textSearch`; (3) 2026-09-07 та
же связка делала ровно один `vectorSearch` на вопрос, значит сломалось что-то в окружении (версия
Ollama/модели), и это отдельная задача. До её решения замерить «четыре инструмента против шести» нельзя,
поэтому дефолт `chatbot.chat.section-tools.enabled: true` остаётся предварительным, а сам флаг и есть
рычаг для этого замера. Проверка самих инструментов сделана на моках: `SectionToolsChatTest` (оглавление,
чтение раздела целиком с цитатой, scope запроса, узость правила корпусного ответа) и `SectionCatalogTest`.
