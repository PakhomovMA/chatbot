...
### Дефект: пустой ответ в детерминированной ветке

`draftAnswer` получает от модели пустой `content`, Jackson падает на
`MismatchedInputException: No content to map due to end-of-input`. Воспроизводится детерминированно: шесть
попыток подряд в одном прогоне (два ретрая биндинга × три ретрая действия), каждая ~15 с, то есть модель
отвечает быстро и пусто. На том же вопросе qwen3 отвечает за 21 с штатно.

Причина **не найдена**. Отвергнуты три гипотезы, каждая пробником по Ollama напрямую:
...
Всего 23 прогона по сырому Ollama — ни одного пустого ответа. Значит различие вносит то, что Embabel
добавляет поверх реконструкции: точная строка формата из `FilteringJacksonOutputConverter` плюс пример от
`WithExampleConverter`. Следующий шаг — получить настоящую строку вызовом `getFormat()` на класспасе
проекта и скормить её Ollama напрямую.
